# Mortgage QA Memory (MQM) — POC / MVP
...
This is the **implementation repo**. The architecture, policy rationale, and
deep-dives live in the sibling design repo (`mcp-memory/`). This repo turns
those docs into runnable npm packages + an MCP server.
...
- **Machine-readable tool contract:** [`docs/tools.json`](./docs/tools.json)
  (regenerate with `npm run manifest`). Point a knowledge MCP or registry at
  this instead of parsing TypeScript.
- **How other tools consume/extend it:** [`docs/INTEGRATION.md`](./docs/INTEGRATION.md)
