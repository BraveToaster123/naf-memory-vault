...
### Naming Conventions
- **Test method**: `TC{StoryID}_Verify{FeatureName}` (NAFLink Profile A)
...
5. If the user didn't specify which ACs/TCs, **ask the user in chat** which ACs or TC IDs to automate (list options from memory). Do not proceed until they confirm.
...
4. If no TC IDs are found, **ask the user in chat** to provide ADO Test Case IDs or type `skip` for placeholders
...
### Phase 6: Human PR gate (MANDATORY)

Do **not** commit or push without explicit user approval.

1. Present a summary: files changed, test method name, build/test result
2. Ask: "Ready to open a PR for human review? (yes/no)"
3. If yes → user creates PR; agent does not force-push or bypass branch policy
