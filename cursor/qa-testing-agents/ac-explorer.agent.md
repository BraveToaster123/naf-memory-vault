...
tools: [microsoft/azure-devops-mcp/wit_get_work_item, microsoft/azure-devops-mcp/wit_get_work_items_batch_by_ids, microsoft/azure-devops-mcp/testplan_list_test_cases, memory-vault/search_nodes, memory-vault/open_nodes, memory-vault/add_observations, memory-vault/create_entities, playwright/browser_navigate, playwright/browser_navigate_back, playwright/browser_click, playwright/browser_type, playwright/browser_fill_form, playwright/browser_select_option, playwright/browser_press_key, playwright/browser_hover, playwright/browser_wait_for, playwright/browser_take_screenshot, playwright/browser_snapshot, sequentialthinking/sequentialthinking]
...
Load from **QA profile** (`qa-profile.yaml` in consumer repo) when present; otherwise defaults below:

- **ADO Project**: `Lender Link Project Management` (profile key: `ado_project`)
- **Application URL**: `https://qa.ll.nafinc.com` (profile key: `app_url`)
- **Credentials**: Resolve from host `credential_ref` (profile key: `login.credential_ref`, e.g. `naflink-qa` → env `CREDENTIAL_REF_NAFLINK_QA`). **Never** store or read passwords from memory-vault.
...
- DO NOT hardcode credentials — resolve from host `credential_ref` / env only
...
5. Resolve login credentials from host env (`CREDENTIAL_REF_*` per QA profile `credential_ref`) — never from memory-vault
