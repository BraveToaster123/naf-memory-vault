...
**Rules:** AC-only actions (no exploratory detours), credentials from host `credential_ref` env (never memory-vault), stop and ask on first failure—no retries.
...
**Lookup order:**
1. **memory-vault** — exploration data, TC IDs, locators
...
**Rules:** Never fabricates data. Always reports the data source (memory-vault / Code / ADO).
...
| `US_{ID}_TestCasesDraft` | testcase-writer | ado-publisher |
...
- **Cursor** with `memory-vault` MCP ([`cursor/mcp.json`](../../cursor/mcp.json)); Flow 1: merge [`cursor/mcp.flow1.json.example`](../../cursor/mcp.flow1.json.example)
- **Optional:** Playwright MCP + Azure DevOps MCP — required for explore/publish pipeline
- **ADO project:** Lender Link Project Management
- **App URL:** `https://qa.ll.nafinc.com` (QA only—never production)
- **Login:** `credential_ref` in [`qa-profile.example.yaml`](../../cursor/qa-profile.example.yaml) — host env resolves secrets (never in memory-vault)
