...
### Step 4: Store draft in memory
7. After writing all test cases, create or update entity **`US_{ID}_TestCasesDraft`** via `memory-vault/create_entities` + `add_observations`:
   - `tc_count: {N}`
   - `tc_{N}_title`, `tc_{N}_ac_index`, `tc_{N}_steps` (ADO Action/Expected format per TC)
8. Do not store credentials or borrower data in observations.

### Step 5: Summary
9. Produce a coverage matrix:
