...
1. Read **`US_{ID}_TestCasesDraft`** from memory-vault (primary source)
2. Fall back to testcase-writer formatted output in chat if draft entity missing
...
9. Store created TC IDs in **`US_{ID}_TestCases`** entity for `automation-generator`
