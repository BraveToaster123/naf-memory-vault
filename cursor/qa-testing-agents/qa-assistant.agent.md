...
## Core Rule: memory-vault First, Then Fetch

For EVERY request, follow this priority order:

### Priority 1: Search memory-vault
1. Extract the **User Story ID** or keywords from the user's question
2. Search memory-vault using `search_nodes` with relevant terms:
...
```
📋 **US {ID} — {Title}**
Source: memory-vault (stored {date})
